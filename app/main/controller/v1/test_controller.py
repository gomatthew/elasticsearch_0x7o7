...
from app.main.service.es_service import set_data, search_data, delete_data
...
class PurchaseOrderBatch(Resource):
    ...
    @api.expect(TestDTO.purchase_order_input)
    def post(self):
        """ 创建数据 """
        resp, status = set_data(request.json, index_name="purchase_order")
        return resp, status
    ...
    def put(self):
        """ 修改数据 """
        resp, status = set_data(request.json, index_name="purchase_order", is_update=True)
        return resp, status

    @api.response(200, '修改成功', model=return_value)
    @api.expect(TestDTO.purchase_order_delete)
    def delete(self):
        """ 删除数据 """
        resp, status = delete_data(request.json, index_name="purchase_order")
        return resp, status
